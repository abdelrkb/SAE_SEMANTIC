...
export interface PythonResultDisplayerProps {
    result: any; // Vous pouvez spécifier un type plus précis si vous savez à quoi ressemble le résultat
}

const PythonResultDisplayer: React.FC<PythonResultDisplayerProps> = ({ result }) => {
    return (
        <div className="PythonResultDisplayer">
            <h3>Python Execution Result</h3>
...
export const WaitingRoomSelector = (props: {rooms: WaitingRoom[], onChosenRoom: (username: string, waitingRoom: string) => void}) => {
    ...
    return <div className="WaintingRoomSelector">
        <div>Votre pseudo: <input type="text" value={username} onChange={event => setUsername(event.target.value)} /></div>
        <div>
        ...
        <button onClick={() => props.onChosenRoom(username, selectedRoom)} disabled={username === "" || selectedRoom === "" || props.rooms.findIndex(x => x.name === selectedRoom) === -1}> Rejoindre la salle d'attente</button>
    </div>;
}
...
export const ChatSession = (props: {messages: Message[], active: boolean, onMessageWritten: (content: string) => void, onLeaving: () => void, onClosing: () => void, onNewGame: () => void}) => {
